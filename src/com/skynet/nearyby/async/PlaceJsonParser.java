...
import com.skynet.nearyby.model.*;
			...
			try {
				...
				if(jsonObject.getString(STATUS).equals("OK")) {
					...
					for(int i = 0; i < results.length(); i++) {
						...
						Log.i(Tags.TAG_GOOGLE_PLACE, "name : " + name + ", lat :" + lat + ", lng : " + lng);
					}
					return placeList;
				} else {
					Log.w(Tags.TAG_GOOGLE_PLACE, "Place API's status code is NOT OK");
					return placeList;
				}
				
			} catch (JSONException e) {
				Log.d(Tags.TAG_GOOGLE_PLACE, e.toString());
				return placeList;
			}
		}
	}
